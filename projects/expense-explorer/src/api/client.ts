export const CONFIG = {
    directBaseUrl: "https://api.tensorlake.ai/v1/namespaces/default",
    proxyBaseUrl: "https://expense-explorer-proxy-bun.onrender.com/api/proxy",
    localProxyUrl: "http://localhost:8888/api/proxy"
};

const POLL_TIMEOUT_MS = 120_000;
const INITIAL_POLL_DELAY_MS = 1_500;
const MAX_POLL_DELAY_MS = 5_000;
const ENABLE_API_DEBUG = import.meta.env.DEV;

interface SessionCredentials {
    tensorlakeKey: string | null;
    geminiKey: string | null;
    databaseUrl: string | null;
}

export function getBaseUrl() {
    const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    if (isLocal) return CONFIG.localProxyUrl;
    return CONFIG.proxyBaseUrl;
}

function getSessionCredentials(): SessionCredentials {
    return {
        tensorlakeKey: sessionStorage.getItem('TENSORLAKE_API_KEY'),
        geminiKey: sessionStorage.getItem('GEMINI_API_KEY'),
        databaseUrl: sessionStorage.getItem('DATABASE_URL')
    };
}

function logApiDebug(message: string) {
    if (ENABLE_API_DEBUG) {
        console.log(message);
    }
}

export function getHeaders(credentials = getSessionCredentials()) {
    const headers: Record<string, string> = {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
    };

    const { tensorlakeKey, geminiKey, databaseUrl } = credentials;

    if (tensorlakeKey) {
        headers['X-TensorLake-API-Key'] = tensorlakeKey;
        headers['Authorization'] = `Bearer ${tensorlakeKey}`;
    }
    if (geminiKey) headers['X-Gemini-API-Key'] = geminiKey;
    if (databaseUrl) headers['X-Database-URL'] = databaseUrl;

    logApiDebug(`[API] Headers Set: TL=${tensorlakeKey ? '✓' : '✗'}, Gemini=${geminiKey ? '✓' : '✗'}, DB=${databaseUrl ? '✓' : '✗'}`);

    return headers;
}

export async function runRemoteApp(appName: string, payload: unknown) {
    const headers = getHeaders(getSessionCredentials());
    const url = `${getBaseUrl()}/applications/${appName}`;
    logApiDebug(`[API] Calling ${appName} at ${url}`);

    const response = await fetch(url, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify(payload)
    });

    if (!response.ok) {
        const errText = await response.text();
        throw new Error(`API Error ${response.status}: ${errText}`);
    }

    return response.json();
}

export async function pollRequest(requestId: string, appName: string) {
    const startTime = Date.now();
    let pollDelayMs = INITIAL_POLL_DELAY_MS;
    const url = `${getBaseUrl()}/applications/${appName}/requests/${requestId}`;
    const headers = getHeaders(getSessionCredentials());

    logApiDebug(`[API] Polling ${appName} request ${requestId} at ${url}...`);

    while (Date.now() - startTime < POLL_TIMEOUT_MS) {
        const response = await fetch(url, {
            headers
        });

        if (!response.ok) {
            throw new Error(`Poll Error: ${response.status} at ${url}`);
        }

        const data = await response.json();

        // TensorLake returns function_runs which contain status
        // We look at the first run (the app itself)
        const mainRun = data.function_runs?.[0];
        const status = mainRun?.status || data.status; // Fallback to root status

        if (status === 'completed') {
            return data.output || mainRun?.output;
        } else if (status === 'failed') {
            throw new Error(data.failure_reason || mainRun?.failure_reason || 'Request failed');
        }

        await new Promise(resolve => setTimeout(resolve, pollDelayMs));
        pollDelayMs = Math.min(MAX_POLL_DELAY_MS, pollDelayMs + 500);
    }

    throw new Error('Request timed out');
}
